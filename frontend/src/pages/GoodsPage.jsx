...
import { getGoods, createGoods, updateGoods, deleteGoods } from '@/services/goodsService';

const EMPTY_FORM = { name: '', sku: '', description: '', unit: '', isActive: true };
...
export default function GoodsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setError(e.response?.data?.message || 'Failed to load goods');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const openEdit = (item) => {
    setEditing(item);
    setForm({ name: item.name, sku: item.sku, description: item.description || '', unit: item.unit || '', isActive: item.isActive });
    setFormError('');
    setShowModal(true);
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">Manage your goods catalogue.</p>
        </div>
        {canWrite && (
      ...
      <div className="card overflow-hidden">
        {loading ? (
          ...
          <div className="p-10 text-center text-gray-400">No goods found. Add some to get started.</div>
        ) : (
          <table className="min-w-full divide-y divide-gray-200 text-sm">
            <thead className="bg-gray-50">
              <tr>
                {['Name', 'SKU', 'Unit', 'Status', ''].map((h) => (
          ...
          <div className="w-full max-w-md rounded-xl bg-white p-6 shadow-xl">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <input className="input" placeholder="e.g. pcs, kg, litre" value={form.unit} onChange={(e) => setForm({ ...form, unit: e.target.value })} />
              </div>
              <div>
